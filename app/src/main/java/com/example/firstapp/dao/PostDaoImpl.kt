...
class PostDaoImpl(private val db: SQLiteDatabase) : PostDao {
    companion object {
        ...
        CREATE TABLE ${PostColumns.TABLE} (
            ...
            ${PostColumns.COLUMN_SHARED_BY_ME} INTEGER NOT NULL DEFAULT 0,
            ${PostColumns.COLUMN_SHARES} INTEGER NOT NULL DEFAULT 0,
            ${PostColumns.COLUMN_VIEWS} INTEGER NOT NULL DEFAULT 0,
            ${PostColumns.COLUMN_VIDEO_ID} TEXT DEFAULT NULL
        );
        """.trimIndent()
    }
    object PostColumns {
        ...
        const val COLUMN_VIDEO_ID = "videoId"
    ...
    private fun map(cursor: Cursor): Post {
        with(cursor) {
            return Post(
                ...
                videoId = getString(getColumnIndexOrThrow(PostColumns.COLUMN_VIDEO_ID))
            )
        }
    }
}